...
#define MMTK_HEAP_COUNT 7
#if SIZEOF_VALUE >= 8
#define MMTK_MAX_OBJ_SIZE 1024
static size_t heap_sizes[MMTK_HEAP_COUNT + 1] = {
    32, 48, 64, 128, 256, 512, MMTK_MAX_OBJ_SIZE, 0
};
#else
#define MMTK_MAX_OBJ_SIZE 512
static size_t heap_sizes[MMTK_HEAP_COUNT + 1] = {
    16, 24, 32, 64, 128, 256, MMTK_MAX_OBJ_SIZE, 0
};
#endif
...
rb_gc_impl_init(void)
{
    ...
    rb_hash_aset(gc_constants, ID2SYM(rb_intern("BASE_SLOT_SIZE")), SIZET2NUM(heap_sizes[0]));
    rb_hash_aset(gc_constants, ID2SYM(rb_intern("RVALUE_SIZE")), SIZET2NUM(heap_sizes[1]));
