            ...
            if (cme) {
                ...
                if (METHOD_ENTRY_CACHED(cme)) {
                    if (METHOD_ENTRY_COMPLEMENTED(cme)) {
                        ...
                        if (METHOD_ENTRY_DEF(cme)->type == VM_METHOD_TYPE_REFINED && !METHOD_ENTRY_DEF(cme)->body.refined.orig_me) {
...
method_definition_release(rb_method_definition_t *def)
{
    if (def != NULL) {
        ...
        /* No free — the def is a GC-managed imemo whose lifetime is determined by reachability. */
        if (METHOD_DEBUG) {
            if (reference_count_was == 1) {
                fprintf(stderr, "-%p-%s:1->0 (release)\n", (void *)def,
                        rb_id2name(def->original_id));
            }
            else {
                fprintf(stderr, "-%p-%s:%d->%d (dec)\n", (void *)def, rb_id2name(def->original_id),
    ...
    if (opts != NULL) {
        switch (def->type) {
          case VM_METHOD_TYPE_ISEQ:
            {
                ...
                RB_OBJ_WRITE((VALUE)me, &def->body.iseq.iseqptr, iseq);
                ...
                RB_OBJ_WRITE((VALUE)me, &def->body.iseq.cref, method_cref);
                return;
            }
          ...
          case VM_METHOD_TYPE_IVAR:
            {
                ...
                if (cfp && (line = rb_vm_get_sourceline(cfp))) {
                    ...
                    RB_OBJ_WRITE((VALUE)me, &def->body.attr.location, location);
                }
                else {
          ...
          case VM_METHOD_TYPE_BMETHOD:
            RB_OBJ_WRITE((VALUE)me, &def->body.bmethod.proc, (VALUE)opts);
          ...
          case VM_METHOD_TYPE_REFINED:
            {
                RB_OBJ_WRITE((VALUE)me, &def->body.refined.orig_me, (rb_method_entry_t *)opts);
                return;
            }
          case VM_METHOD_TYPE_ALIAS:
            RB_OBJ_WRITE((VALUE)me, &def->body.alias.original_me, (rb_method_entry_t *)opts);
            return;
...
method_definition_reset(const rb_method_entry_t *me)
{
    ...
    switch (def->type) {
      case VM_METHOD_TYPE_ISEQ:
        RB_OBJ_WRITTEN((VALUE)me, Qundef, def->body.iseq.iseqptr);
        RB_OBJ_WRITTEN((VALUE)me, Qundef, def->body.iseq.cref);
        break;
      case VM_METHOD_TYPE_ATTRSET:
      case VM_METHOD_TYPE_IVAR:
        RB_OBJ_WRITTEN((VALUE)me, Qundef, def->body.attr.location);
        break;
      case VM_METHOD_TYPE_BMETHOD:
        RB_OBJ_WRITTEN((VALUE)me, Qundef, def->body.bmethod.proc);
        break;
      case VM_METHOD_TYPE_REFINED:
        RB_OBJ_WRITTEN((VALUE)me, Qundef, def->body.refined.orig_me);
        break;
      case VM_METHOD_TYPE_ALIAS:
        RB_OBJ_WRITTEN((VALUE)me, Qundef, def->body.alias.original_me);
        break;
      case VM_METHOD_TYPE_CFUNC:
      case VM_METHOD_TYPE_ZSUPER:
      case VM_METHOD_TYPE_MISSING:
...
static void
method_definition_init(rb_method_definition_t *def, rb_method_type_t type, ID mid)
{
...
rb_method_entry_alloc(ID called_id, VALUE owner, VALUE defined_class, rb_method_type_t type)
{
    ...
    /* Zero everything after RBasic so the GC marker never sees stale data
     * from a prior slot occupant (the debug allocator fills with 0xFF). */
    memset((char *)me + sizeof(VALUE) * 2, 0, sizeof(rb_method_entry_t) - sizeof(VALUE) * 2);

    me->called_id = called_id;
    me->owner = owner;

    if (type) {
        method_definition_init(&me->def, type, called_id);
    }
...
rb_method_entry_t *
rb_method_entry_create(ID called_id, VALUE klass, rb_method_visibility_t visi, rb_method_type_t type)
{
    rb_method_entry_t *me = rb_method_entry_alloc(called_id, klass, filter_defined_class(klass), type);
    METHOD_ENTRY_FLAGS_SET(me, visi, ruby_running ? FALSE : TRUE);
    if (type) method_definition_reset(me);
    return me;
}
...
{
    rb_method_definition_t *src_def = METHOD_ENTRY_DEF(src_me);
    rb_method_type_t type = src_def ? src_def->type : (rb_method_type_t)0;
    rb_method_entry_t *me = rb_method_entry_alloc(src_me->called_id, src_me->owner, src_me->defined_class, type);
    if (src_def) {
        memcpy(&me->def, src_def, sizeof(rb_method_definition_t));
        method_definition_reset(me);
        src_def->aliased = true;
    }
    METHOD_ENTRY_FLAGS_COPY(me, src_me);

    if (src_def &&
            src_def->type == VM_METHOD_TYPE_REFINED &&
            src_def->body.refined.orig_me) {
...
{
    rb_method_definition_t *def = METHOD_ENTRY_DEF(src_me);
    ...
    me = rb_method_entry_alloc(called_id, src_me->owner, defined_class,
                               def ? def->type : VM_METHOD_TYPE_REFINED);
    if (def) {
        memcpy(&me->def, def, sizeof(rb_method_definition_t));
        method_definition_reset(me);
    }
    else {
        rb_method_definition_set(me, &me->def, (void *)refined_orig_me);
    }
    METHOD_ENTRY_FLAGS_COPY(me, src_me);
...
{
    memcpy(&dst->def, &src->def, sizeof(rb_method_definition_t));
    method_definition_reset(dst);
    ((rb_method_entry_t *)src)->def.aliased = true;
                     ...
                     rb_method_type_t type, rb_method_definition_t *def, ID original_id, void *opts)
{
    ...
    if (RB_TYPE_P(klass, T_MODULE) && FL_TEST(klass, RMODULE_IS_REFINEMENT)) {
        ...
        if (search_superclass) {
            rb_method_entry_t *me = lookup_method_table(refined_class, mid);
            RB_OBJ_WRITE((VALUE)me, &METHOD_ENTRY_DEF(me)->body.refined.orig_me, (VALUE)search_method0(refined_class, mid, NULL, true));
        }
    }
    if (type == VM_METHOD_TYPE_REFINED) {
    ...
    if (rb_id_table_lookup(mtbl, mid, &data)) {
        ...
        rb_method_definition_t *old_def = METHOD_ENTRY_DEF(old_me);
    ...
    /* create method entry */
    me = rb_method_entry_create(mid, defined_class, visi, type);
    if (def) {
        /* Copy an existing definition into the new ME */
        memcpy(&me->def, def, sizeof(rb_method_definition_t));
        method_definition_reset(me);
        def->aliased = true;
    }
        ...
        if (original_id != mid) {
            me->def.original_id = original_id;
        }
        rb_method_definition_set(me, &me->def, opts);
    }
...
get_overloaded_cme(const rb_callable_method_entry_t *cme)
{
    ...
    if (monly_cme && !METHOD_ENTRY_INVALIDATED(monly_cme)) {
        ...
        rb_method_entry_t *me = rb_method_entry_alloc(cme->called_id,
                                                      ...
                                                      cme->defined_class,
                                                      VM_METHOD_TYPE_ISEQ);
        me->def.original_id = METHOD_ENTRY_DEF(cme)->original_id;

        RB_OBJ_WRITE((VALUE)me, &me->def.body.iseq.cref, METHOD_ENTRY_DEF(cme)->body.iseq.cref);
        RB_OBJ_WRITE((VALUE)me, &me->def.body.iseq.iseqptr, ISEQ_BODY(METHOD_ENTRY_DEF(cme)->body.iseq.iseqptr)->mandatory_only_iseq);
                 ...
                 rb_method_visibility_t visi, VALUE defined_class)
{
    ...
    RB_VM_LOCKING() {
        newme = rb_method_entry_make(klass, mid, defined_class, visi,
                METHOD_ENTRY_DEF(me)->type, METHOD_ENTRY_DEF(me), 0, NULL);
        if (newme == me) {
            METHOD_ENTRY_DEF(me)->no_redef_warning = TRUE;
...
search_method0(VALUE klass, ID id, VALUE *defined_class_ptr, bool skip_refined)
{
    ...
    for (; klass; klass = RCLASS_SUPER(klass)) {
        ...
        if ((me = lookup_method_table(klass, id)) != 0) {
            if (!skip_refined || METHOD_ENTRY_DEF(me)->type != VM_METHOD_TYPE_REFINED ||
                    METHOD_ENTRY_DEF(me)->body.refined.orig_me) {
                break;
            }
        }
    }
...
negative_cme(ID mid)
{
    ...
    if (rb_id_table_lookup(&vm->negative_cme_table, mid, &cme_data)) {
        ...
        cme = (rb_callable_method_entry_t *)rb_method_entry_alloc(mid, Qnil, Qnil, (rb_method_type_t)0);
...
callable_method_entry_refinements0(VALUE klass, ID id, VALUE *defined_class_ptr, bool with_refinements,
                                    const rb_callable_method_entry_t *cme)
{
    if (cme == NULL || LIKELY(METHOD_ENTRY_DEF(cme)->type != VM_METHOD_TYPE_REFINED)) {
        return cme;
    }
    else {
...
{
    while (me && METHOD_ENTRY_DEF(me)->type == VM_METHOD_TYPE_REFINED) {
        VALUE refinement;
        ...
        if (!NIL_P(refinement)) {
            ...
            if (tmp_me && METHOD_ENTRY_DEF(tmp_me)->type != VM_METHOD_TYPE_REFINED) {
                return tmp_me;
            }
        }

        tmp_me = METHOD_ENTRY_DEF(me)->body.refined.orig_me;
        if (tmp_me) {
...
remove_method(VALUE klass, ID mid)
{
    ...
    if (!rb_id_table_lookup(RCLASS_M_TBL(klass), mid, &data) ||
        ...
        (!METHOD_ENTRY_DEF(me) || METHOD_ENTRY_DEF(me)->type == VM_METHOD_TYPE_UNDEF) ||
        UNDEFINED_REFINED_METHOD_P(METHOD_ENTRY_DEF(me))) {
        ...
        if (klass == defined_class || origin_class == defined_class) {
            if (METHOD_ENTRY_DEF(me)->type == VM_METHOD_TYPE_REFINED) {
                ...
                if (METHOD_ENTRY_DEF(me)->body.refined.orig_me) {
                    METHOD_ENTRY_VISI_SET((rb_method_entry_t *)METHOD_ENTRY_DEF(me)->body.refined.orig_me, visi);
                }
            }
            else {
...
method_boundp(VALUE klass, ID id, int ex)
{
    ...
    if (cme != NULL) {
        ...
        if (METHOD_ENTRY_DEF(cme)->type == VM_METHOD_TYPE_NOTIMPLEMENTED) {
...
rb_undef(VALUE klass, ID id)
{
    ...
    if (me && METHOD_ENTRY_DEF(me)->type == VM_METHOD_TYPE_REFINED) {
...
check_definition_visibility(VALUE mod, int argc, VALUE *argv)
{
    ...
    if (me) {
        if (METHOD_ENTRY_DEF(me)->type == VM_METHOD_TYPE_NOTIMPLEMENTED) return METHOD_VISI_UNDEF;
...
{
    return rb_method_definition_eq(METHOD_ENTRY_DEF(m1), METHOD_ENTRY_DEF(m2));
}
        ...
        switch (def->type) {
          case VM_METHOD_TYPE_REFINED:
            if (def->body.refined.orig_me) {
                def = METHOD_ENTRY_DEF(def->body.refined.orig_me);
                goto again;
            }
            break;
          case VM_METHOD_TYPE_ALIAS:
...
{
    return rb_hash_method_definition(hash, METHOD_ENTRY_DEF(me));
}
...
rb_alias(VALUE klass, ID alias_name, ID original_name)
{
  ...
  again:
    ...
    if (orig_me && METHOD_ENTRY_DEF(orig_me)->type == VM_METHOD_TYPE_REFINED) {
        orig_me = rb_resolve_refined_method(Qnil, orig_me);
    }

    if (UNDEFINED_METHOD_ENTRY_P(orig_me) ||
        UNDEFINED_REFINED_METHOD_P(METHOD_ENTRY_DEF(orig_me))) {
    ...
    switch (METHOD_ENTRY_DEF(orig_me)->type) {
      case VM_METHOD_TYPE_ZSUPER:
        klass = RCLASS_SUPER(klass);
        original_name = METHOD_ENTRY_DEF(orig_me)->original_id;
        visi = METHOD_ENTRY_VISI(orig_me);
        goto again;
      case VM_METHOD_TYPE_ALIAS:
        visi = METHOD_ENTRY_VISI(orig_me);
        orig_me = METHOD_ENTRY_DEF(orig_me)->body.alias.original_me;
        VM_ASSERT(METHOD_ENTRY_DEF(orig_me)->type != VM_METHOD_TYPE_ALIAS);
        break;
      default: break;
    }
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (module == defined_class || origin_class == defined_class) {
            switch (METHOD_ENTRY_DEF(me)->type) {
              case VM_METHOD_TYPE_ISEQ:
                if (ISEQ_BODY(METHOD_ENTRY_DEF(me)->body.iseq.iseqptr)->param.flags.has_rest &&
                        !ISEQ_BODY(METHOD_ENTRY_DEF(me)->body.iseq.iseqptr)->param.flags.has_post &&
                        !ISEQ_BODY(METHOD_ENTRY_DEF(me)->body.iseq.iseqptr)->param.flags.has_kw &&
                        !ISEQ_BODY(METHOD_ENTRY_DEF(me)->body.iseq.iseqptr)->param.flags.has_kwrest) {
                    ISEQ_BODY(METHOD_ENTRY_DEF(me)->body.iseq.iseqptr)->param.flags.ruby2_keywords = 1;
                ...
                break;
              case VM_METHOD_TYPE_BMETHOD: {
                VALUE procval = METHOD_ENTRY_DEF(me)->body.bmethod.proc;
